...
export function OrderDashboard() {
  ...
  return <section className="section simple-page"><p className="eyebrow">Your journey</p><h1>Orders</h1><div className="cart-items" style={{ display: 'grid', gap: '16px' }}>{orders.map((order) => <div className="cart-item" key={order.id}><div className="cart-item-copy"><h3>Order #{order.id.slice(-6)}</h3><p>{order.product} × {order.quantity}</p><strong>{money(order.total)}</strong><p>Status: {order.status} · Payment: {order.paymentStatus}</p><p>Placed: {new Date(order.createdAt).toLocaleDateString('en-IN')}</p>{order.estimatedDeliveryDate && <p>Expected delivery: {new Date(order.estimatedDeliveryDate).toLocaleDateString('en-IN', { dateStyle: 'long' })}</p>}{order.delivery && <p>Delivery: {order.delivery.address || `${order.delivery.city}, ${order.delivery.state}`}</p>}</div></div>)}</div></section>
}
